...
- [ ] جميع المسارات المذكورة في قسم API Contract تعمل.
- [ ] لا يستطيع المستخدم التسجيل مباشرة كـAdmin أو Instructor.
- [ ] رحلة `register → verify → login → refresh → logout` مغطاة باختبارات Integration.
- [ ] رحلة `apply → admin approve/reject → create draft course` مغطاة باختبارات Integration.
- [ ] لا يستطيع محاضر قراءة أو تعديل كورسات محاضر آخر.
- [ ] حذف Section يحذف Lessons التابعة له بصورة متسقة.
- [ ] إعادة الترتيب لا تسمح بمعرّفات ناقصة أو زائدة أو مكررة.
- [ ] البريد يعمل في بيئة التطوير أو الاختبار من خلال مزود محدد.
- [ ] تجربة رفع فيديو مباشر تعمل، والـWebhook يتحقق من التوقيع قبل تعديل البيانات.
- [ ] الأخطاء ترجع JSON موحدًا ولا تكشف stack trace أو أسرارًا في الإنتاج.
- [ ] `.env.example` وREADME وAPI collection/OpenAPI محدثة.
- [ ] الاختبارات وlint ينجحان من أمر واحد موثق.
